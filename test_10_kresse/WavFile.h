 ...
 * Version 7: merged stuff i found. this should support "long files" and have the correct range
 * Version 6: fixed a bug where i used delete instead of delete[]
 * Version 5
 */
#include <stdio.h>
...
public:
	...
	void writeFormat() {
		...
		short myBlockAlign = (short)(channels * bitsPerSample/8);
		
		uint32_t myChunkSize = 36 + length*bitsPerSample/8;
		uint32_t myDataSize = length*bitsPerSample/8;
	...
	bool write(float *samples, const int buffLength) {
		...
		for(int i = 0; i < buffLength; i++) {
			...
			if( samp > 1 ) samp = 1;
			if( samp < -1 ) samp = -1;
			buff[i] = (short)round(samp*32767.f);
		}
...
#define WRITE_BUFF_SIZE 256
	...
	bool save(string path) {
		...
		while(pos<length) {
			...
			for(int i = 0; i < len; i++) {
				buff[i] = (short)roundf(data[pos]*32760.f);
				pos++;
			}
