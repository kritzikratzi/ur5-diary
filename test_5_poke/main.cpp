...
int main(int argc, char* argv[])
{
	...
	std::vector<int> selection_vector = {0, 1, 0, 0, 0, 0};
	std::vector<double> wrench_down = {0, -80, 0, 0, 0, 0};
	std::vector<double> wrench_up = {0, 40, 0, 0, 0, 0};
	int force_type = 2;
	std::vector<double> limits = {2, 2, 1.5, 1, 1, 1};
	
	
	auto watch_force = [&](int millis, int axis){
		int count = 0;
		...
		for(int i = 0; i < millis-50; i++){
			...
			if(count % 30 == 0){
				cout << force[axis] << endl;
			}
			count ++;
			
			if(abs(force[axis])>=8){
				cout << "abort with" << force[axis] << endl;
				break;
			}
		}
	};
